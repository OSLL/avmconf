/****************************************************************************
** Meta object code from reading C++ file 'tests.h'
**
** Created: Tue Apr 3 15:57:20 2012
**      by: The Qt Meta Object Compiler version 62 (Qt 4.6.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "tests.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'tests.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.6.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_Tests[] = {

 // content:
       4,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
       7,    6,    6,    6, 0x08,
      20,    6,    6,    6, 0x08,
      32,    6,    6,    6, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_Tests[] = {
    "Tests\0\0simpleTest()\0stateTest()\0"
    "manyContainersTest()\0"
};

const QMetaObject Tests::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_Tests,
      qt_meta_data_Tests, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &Tests::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *Tests::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *Tests::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Tests))
        return static_cast<void*>(const_cast< Tests*>(this));
    return QObject::qt_metacast(_clname);
}

int Tests::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: simpleTest(); break;
        case 1: stateTest(); break;
        case 2: manyContainersTest(); break;
        default: ;
        }
        _id -= 3;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
