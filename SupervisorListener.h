...
protected:
	...
	for (;;) {
	    ...
	    if (ev->event == ANDCONT_SUPERV_EVENT_FOREGROUND_CHANGED) {
		if ((pid_t)ev->arg == 1) {
		    __LOGE("Switched to the root\n");

		    emit switchedToRoot();
		} else {
		    __LOGE("Switched to a container\n");

		    emit switchedToContainer();
		}
	    }
